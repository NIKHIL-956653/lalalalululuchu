...
export function makeAIMove(board, playerIndex, difficulty, rows, cols, playersLength) {
  ...
  // NEW "EASY (SMART)" MODE
  // This uses the Heuristic logic (formerly Medium). It plays strategically but doesn't calculate deep futures.
  ...
  if (difficulty === "hard") {
    ...
    if (candidates.length > 40) return makeAIMove(board, playerIndex, "easy", rows, cols, playersLength);
    ...
    return bestMove || makeAIMove(board, playerIndex, "easy", rows, cols, playersLength);
  }
}